...
class IndicatorProcessorServiceTest {
    ...
    void processLatestIndicatorsShouldPassCandlesInChronologicalOrderToPatternEngine() {
        ...
        when(candleRepository.findDistinctSymbols()).thenReturn(List.of(symbol));
        when(candleRepository.findTop500BySymbolAndTimeframeOrderByCandleTimeDesc(symbol, "ONE_MINUTE"))
                .thenReturn(List.of(newest, oldest));
        when(barSeriesManager.getSeries(symbol, "ONE_MINUTE")).thenReturn(null);
        when(indicatorService.calculateIndicators(eq(symbol), eq("ONE_MINUTE"), eq("BTC"), eq(newest.getCandleTime()), anyList()))
                .thenReturn(mock(IndicatorResultDto.class));
        when(patternEngine.detectPattern(anyString(), anyList())).thenReturn(PatternResult.none());

        indicatorProcessorService.processLatestIndicators();
