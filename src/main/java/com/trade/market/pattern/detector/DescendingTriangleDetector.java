...
public class DescendingTriangleDetector implements PatternDetector {
    ...
    public boolean detect(List<Candle> candles) {
        ...
        log.info("Descending Triangle detected: Support={}, Resistance={}", 
            String.format("%.2f", support),
            String.format("%.2f", swingHighs.get(swingHighs.size() - 1).getPrice()));
    ...
    public PatternResult detectWithResult(List<Candle> candles) {
        ...
        Candle lastCandle = candles.get(candles.size() - 1);
        int confidence = calculateConfidence(candles, swingHighs, swingLows, support, lastCandle);
    ...
    private int calculateConfidence(List<Candle> candles, List<PivotPoint> swingHighs,
                                    List<PivotPoint> swingLows, double support, Candle lastCandle) {
        int confidence = 50;
